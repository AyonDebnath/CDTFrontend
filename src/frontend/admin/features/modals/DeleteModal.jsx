...
export default function DeleteModal({ curUser }) {
  const del = useContext(DelContext);
  ...
  const handleSubmit = async (event) => {
    ...
    if (formState.inputs.name.value === curUser.fname) {
      try {
        await sendRequest(
          `${import.meta.env.VITE_SERVER_NAME}api/admin/user/delete/${
            curUser.id
          }`,
          "DELETE",
          null,
        ...
        {curUser && (
          <div className="container">
            <div className="d-flex justify-content-center">
              ...
              >
                <Label>
                  Enter the user name <strong>{curUser.fname}</strong> to delete
                  the user
                </Label>
